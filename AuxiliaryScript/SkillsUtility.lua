		...
		if an == 'pudge_meat_hook' or an == 'rattletrap_hookshot' then
			...
			then
				...
				then
					...
					if not AreCreepsBetweenMeAndLoc(pLoc, nRadius)  then
		...
		elseif 	an == 'mirana_arrow' then
			...
			then
				...
				then
					local distance = GetUnitToUnitDistance(npcTarget, npcBot)
					local pLoc = npcTarget:GetExtrapolatedLocation( nCastPoint + ( distance / speed ) );
					if not AreEnemyCreepsBetweenMeAndLoc(pLoc, nRadius)  then
						return BOT_ACTION_DESIRE_MODERATE, pLoc;
					end
				end
			end
...
function AreCreepsBetweenMeAndLoc(loc, lineOfSightThickness)
    if not AreEnemyCreepsBetweenMeAndLoc(loc, lineOfSightThickness) then
        return AreFriendlyCreepsBetweenMeAndLoc(loc, lineOfSightThickness)
    end
    return true
end

function AreEnemyCreepsBetweenMeAndLoc(loc, lineOfSightThickness)
    local npcBot = GetBot()
    ...
    for _, eCreep in ipairs(eCreeps) do
        ...
        if x ~= y then
            ...
            if d <= lineOfSightThickness and
                GetUnitToLocationDistance(npcBot, loc) > (GetDistance(x,loc) + 50) then
                return true
            end
        end
    end
...
function AreFriendlyCreepsBetweenMeAndLoc(loc, lineOfSightThickness)
    local npcBot = GetBot()
