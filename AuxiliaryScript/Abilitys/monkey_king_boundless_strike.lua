...
--初始数据
local ability = bot:GetAbilityByName('monkey_king_boundless_strike')
...
function X.Consider()
	...
	if bot:HasModifier('modifier_monkey_king_jingu_mastery') then
		local tableNearbyEnemyHeroes = bot:GetNearbyHeroes( nCastRange , true, BOT_MODE_NONE );
		for _,npcEnemy in pairs( tableNearbyEnemyHeroes )
		do
			if (J.CanCastOnNonMagicImmune(npcEnemy) and J.IsInRange(npcEnemy, bot, nCastRange-200)) 
			then
				--return BOT_ACTION_DESIRE_HIGH, npcEnemy:GetLocation( );
				return BOT_ACTION_DESIRE_MODERATE, npcEnemy:GetExtrapolatedLocation( nCastPoint );
			end
