...
--初始数据
local ability = bot:GetAbilityByName('venomancer_plague_ward')
...
function X.Consider()
	...
	if ( bot:GetActiveMode() == BOT_MODE_LANING and
		bot:GetMana()/bot:GetMaxMana() >= 0.75 )
	then
	...
	if J.IsDefending(bot) or J.IsPushing(bot) and bot:GetMana()/bot:GetMaxMana() >= 0.55
	then
		local tableNearbyEnemyCreeps = bot:GetNearbyLaneCreeps( 1000, true);
