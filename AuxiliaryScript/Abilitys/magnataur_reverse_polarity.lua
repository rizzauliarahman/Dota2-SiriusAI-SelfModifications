...
--初始数据
local ability = bot:GetAbilityByName('magnataur_reverse_polarity')
...
function X.Consider()
	...
	local nRadius = ability:GetSpecialValueInt( "pull_radius" );
	local nCastRange = 0;
	local nDamage = ability:GetSpecialValueInt("polarity_damage");
	...
	if J.IsRetreating(bot) and  ( not bot:HasModifier("modifier_magnataur_skewer_movement") or not abilityTW:IsInAbilityPhase() )
	then
		local tableNearbyEnemyHeroes = bot:GetNearbyHeroes( nRadius-100, true, BOT_MODE_NONE );
		local tableNearbyAllyHeroes = bot:GetNearbyHeroes( 800, false, BOT_MODE_ATTACK );
	...
	then
		...
		local nInvUnit = X.CountInvUnits(true, tableNearbyEnemyHeroes);
		if nInvUnit >= 2 then
			return BOT_ACTION_DESIRE_HIGH;
		end
	end
