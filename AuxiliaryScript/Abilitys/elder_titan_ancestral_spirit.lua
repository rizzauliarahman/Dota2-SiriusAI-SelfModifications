...
--初始数据
local ability = bot:GetAbilityByName('elder_titan_ancestral_spirit')
...
function X.Consider()
	...
	local nDamage = ability:GetSpecialValueInt("pass_damage");
	...
	then
		...
		local locationHAoE = bot:FindAoELocation( true, true, bot:GetLocation(), nCastRange, nRadius, 0, 0 );
		if ( locationHAoE.count >= 2  ) 
		then
			return BOT_ACTION_DESIRE_LOW, locationHAoE.targetloc;
		end
		local locationCAoE = bot:FindAoELocation( true, false, bot:GetLocation(), nCastRange, nRadius, 0, 0 );
