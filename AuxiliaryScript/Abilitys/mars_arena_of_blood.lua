...
--初始数据
local ability = bot:GetAbilityByName('mars_arena_of_blood')
...
function X.Consider()
	...
	local nDamage   = ability:GetSpecialValueInt('spear_damage');
	...
	then
		...
		then
			...
			if #targetAllies >= 2 then
				return BOT_ACTION_DESIRE_HIGH, X.GetProperLocation( target, castPoint );
			end
		end
	end
