...
--初始数据
local ability = bot:GetAbilityByName('ember_spirit_flame_guard')
...
function X.Consider()
	...
	local nDamage   = ability:GetSpecialValueFloat( "duration" ) * ability:GetSpecialValueInt( "damage_per_second" )
	...
	then
		...
		if J.IsValidHero(npcTarget) and J.CanCastOnNonMagicImmune(npcTarget) and J.IsInRange(npcTarget, bot, nRadius + 200)
		then
