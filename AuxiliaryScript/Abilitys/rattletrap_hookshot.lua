...
--初始数据
local ability = bot:GetAbilityByName('rattletrap_hookshot')
...
function X.Consider()
	...
	local nCastRange2 = 1500 + ability:GetLevel()*500;
	...
	then
		...
		for i=1, #allies do
			...
			then
				...
				if IsHeroBetweenMeAndTarget(bot, allies[i], pLoc, nRadius) == false 
					and J.IsCreepBetweenMeAndTarget(bot, allies[i], pLoc, nRadius) == false
				then
		...
		for i=1, #camps do
			local cDist = J.Site.GetDistance(camps[i].location, loc); 
			if cDist < bDist 
	...
	then
		...
		if  J.IsValidHero(target) and J.CanCastOnMagicImmune(target) and J.IsInRange(target, bot, nCastRange-150)
		then
