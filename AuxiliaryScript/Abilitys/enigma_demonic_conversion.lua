...
--初始数据
local ability = bot:GetAbilityByName('enigma_demonic_conversion')
...
function X.Consider()
	...
	then
		if bot:GetMana()/bot:GetMaxMana() > 0.65 then
			...
			do
				if J.CanCastOnNonMagicImmune(npcACreep)  
				then
	...
	then
		...
		if J.IsValidHero(npcTarget) and J.IsInRange(npcTarget, bot, nCastRange+200) and bot:GetMana()/bot:GetMaxMana() > 0.65  
		then
			local tableNearbyAlliedCreeps = bot:GetNearbyLaneCreeps ( nCastRange, false );
			local tableNearbyEnemyCreeps = bot:GetNearbyLaneCreeps ( nCastRange, true );
			for _,npcECreep in pairs( tableNearbyEnemyCreeps  )
