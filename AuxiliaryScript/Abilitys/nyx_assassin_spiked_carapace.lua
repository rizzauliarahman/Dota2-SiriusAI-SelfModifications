...
--初始数据
local ability = bot:GetAbilityByName('nyx_assassin_spiked_carapace')
...
function X.Consider()
	...
	then
		...
		do
			...
			if enemy:IsUsingAbility() or enemyTarget == bot then
				return BOT_ACTION_DESIRE_MODERATE;
			end
		end
	end
	...
	then
		...
		if J.IsValidHero(npcTarget) and J.CanCastOnNonMagicImmune(npcTarget) and J.IsInRange(npcTarget, bot, 600) and npcTarget:IsUsingAbility( )  
		then
			return BOT_ACTION_DESIRE_MODERATE;
		end
	end
