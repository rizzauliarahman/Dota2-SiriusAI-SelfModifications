...
--初始数据
local ability = bot:GetAbilityByName('tinker_laser')
...
function X.Consider()
	...
	local nDamage = ability:GetSpecialValueInt("laser_damage");
	local currManaP = bot:GetMana() / bot:GetMaxMana();
	--------------------------------------
	-- Mode based usage
	--------------------------------------
	if bot:GetActiveMode() == BOT_MODE_LANING and currManaP > 0.55  
	then
