...
--初始数据
local ability = bot:GetAbilityByName('shredder_return_chakram_2')
...
function X.Release(castTarget)
	...
	bot.ultLoc2 = Vector(-6376, 6419, 0)
end
...
function X.Consider()
    ...
    if ability == nil
       ...
       or ability:IsHidden()
       or DotaTime() < bot.ultTime2 + bot.ultETA2 or StillTraveling(2)
	then 
	...
	if bot:GetMana() < 100 or GetUnitToLocationDistance(bot, bot.ultLoc2) > 1600 then
	...
	then
		local nUnits = 0;
		local nLowHPUnits = 0;
		local NearbyUnits = bot:GetNearbyLaneCreeps(1000, true);
		for _,c in pairs(NearbyUnits)
		do 
			if GetUnitToLocationDistance(c, bot.ultLoc2) < nRadius  then
				nUnits = nUnits + 1;
			end
			if GetUnitToLocationDistance(c, bot.ultLoc2) < nRadius and c:GetHealth() <= nDamage then
	...
	then
		...
		do 
			...
			if GetUnitToLocationDistance(c, bot.ultLoc2) < nRadius and c:GetHealth() <= nDamage / 2 then
				nLowHPUnits = nLowHPUnits + 1;
			end
		end
		--print("Attck"..nUnits)
