...
--初始数据
local ability = bot:GetAbilityByName('tinker_march_of_the_machines')
...
function X.Consider()
	...
	then
		...
		if ( locationAoE.count >= 3 and (bot:GetMana() / bot:GetMaxMana()) > 0.45 ) 
		then
			return BOT_ACTION_DESIRE_LOW, U.GetXUnitsInFront(nCastRange/2);
		end
	end
	...
	then
		...
		then
			return BOT_ACTION_DESIRE_MODERATE, U.GetXUnitsInFront(nCastRange/2);
		end
	end
