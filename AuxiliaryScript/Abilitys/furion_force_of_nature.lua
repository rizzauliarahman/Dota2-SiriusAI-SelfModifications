...
--初始数据
local ability = bot:GetAbilityByName('furion_force_of_nature')
...
function X.Consider()
	...
	then
		...
		then
			local nearbyTrees = bot:GetNearbyTrees(nCastRange + nRadius)
	...
	then
		...
		if J.IsValidHero(npcTarget) and J.IsInRange(npcTarget, bot, 1200)
		then
			local nearbyTrees = bot:GetNearbyTrees(nCastRange + nRadius)
			if nearbyTrees[1] ~= nil then
				return BOT_ACTION_DESIRE_MODERATE, GetTreeLocation(nearbyTrees[1])
