...
--初始数据
local ability = bot:GetAbilityByName('chen_divine_favor')
...
function X.Consider()
	...
	then
		local nearbyTower = bot:GetNearbyTowers(1000, false) 
	...
	if J.IsPushing(bot)
	then
		local nearbyTower = bot:GetNearbyTowers(1000, true) 
		if nearbyTower[1] ~= nil then
			local maxDist = 0;
			local target = nil;
			for i = 1, #numPlayer
	...
	then
		...
		if ( npcTarget ~= nil  and npcTarget:IsHero() and GetUnitToUnitDistance( npcTarget, bot ) < 1600  ) 
		then	
			local maxDist = 0;
			local target = nil;
			for i = 1, #numPlayer
			do
				local player = GetTeamMember(i);
				if player ~= nil and player:IsAlive() and player:GetActiveMode() ~= BOT_MODE_RETREAT then
					local dist = GetUnitToUnitDistance(player, bot);
					local health = player:GetHealth()/player:GetMaxHealth();
					if IsPlayerBot(player:GetPlayerID()) and dist > maxDist and dist > 4000 and health >= 0.25 then
						maxDist = dist;
						target = GetTeamMember(i);
