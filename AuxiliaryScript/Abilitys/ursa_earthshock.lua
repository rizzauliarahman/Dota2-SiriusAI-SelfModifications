...
--初始数据
local ability = bot:GetAbilityByName('ursa_earthshock')
...
function X.Consider()
	...
	local nRadius = ability:GetSpecialValueInt( "shock_radius" );
	local nCastRange = 0;
	...
	then
		...
		if J.IsValidHero(npcTarget) and J.CanCastOnNonMagicImmune(npcTarget) and J.IsInRange(npcTarget, bot, nRadius - 150)
		then
			return BOT_ACTION_DESIRE_MODERATE;
		end
	end
