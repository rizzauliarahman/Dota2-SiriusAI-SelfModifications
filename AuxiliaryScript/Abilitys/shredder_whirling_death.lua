...
--初始数据
local ability = bot:GetAbilityByName('shredder_whirling_death')
...
function X.Consider()
	...
	local nRadius = ability:GetSpecialValueInt( "whirling_radius" );
	local nCastRange = 0;
	local nDamage = ability:GetSpecialValueInt("whirling_damage");
	...
	then
		...
		if NearbyCreeps ~= nil and #NearbyCreeps >= 3 and bot:GetMana()/bot:GetMaxMana() > 0.65 then 
