...
--初始数据
local ability = bot:GetAbilityByName('keeper_of_the_light_chakra_magic')
...
function X.Consider()
	...
	if  bot:GetMana() / bot:GetMaxMana() < 0.75 then
		return BOT_ACTION_DESIRE_MODERATE, bot;
	else
		local tableNearbyFriendlyHeroes = bot:GetNearbyHeroes( 1200, false, BOT_MODE_NONE );
		for _,myFriend in pairs(tableNearbyFriendlyHeroes) do
			if ( J.CanCastOnNonMagicImmune(myFriend) and myFriend:GetMana() / myFriend:GetMaxMana() < 0.65  ) 
			then
