...
--初始数据
local ability = bot:GetAbilityByName('beastmaster_call_of_the_wild_boar')
local abilityQ = bot:GetAbilityByName('beastmaster_wild_axes')
...
function X.Consider()
	...
	then
		...
		if J.IsValidHero(target) and J.CanCastOnMagicImmune(target) and J.IsInRange(target, bot, 1000)
		then
			return BOT_ACTION_DESIRE_HIGH;
		end
	end
