...
--初始数据
local ability = bot:GetAbilityByName('ember_spirit_searing_chains')
...
function X.Consider()
	...
	local nDamage   = ability:GetSpecialValueInt( "total_damage_tooltip" );
	...
	then
		...
		if J.IsValidHero(npcTarget) and J.CanCastOnNonMagicImmune(npcTarget) and J.IsInRange(npcTarget, bot, nRadius - 50)
		then
			return BOT_ACTION_DESIRE_HIGH;
		end
	end
