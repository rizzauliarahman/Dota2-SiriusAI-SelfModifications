...
if pcall( require,  'game/bot_omgHero' )
then
	local omgHeroList = require( 'game/bot_omgHero' )
	for i = 1, #omgHeroList
	do 
		sBanList[#sBanList + 1] = omgHeroList[i]	
	end
end
...
function AllPickLogic()	
	...
	do
		...
		then
			...
			--if i == 2 then sSelectHero = "npc_dota_hero_bane" end 
			------------------------------------------------------------------------------------
			--新版英雄选择策略
