...
local sNoticeList = {
	...
	[2] = "赞助冠名或吐槽建议,请联系:dota2jmz@163.匚om",
	...
	[7] = "暂停功能会导致客户端BUG引发AI重复出装的问题.",
	[8] = "我们正处于各种困境之中, 请帮我们出谋划策.",
	[9] = "由于客户端不再开放经验补丁功能, 翻倍补丁已失效.",
	[10] = "由于客户端不再开放经验补丁功能, 翻倍补丁已失效.",
...
function GetDesire()	
	...
	InstallChatCallback(function ( tChat )
		...
		elseif tChat.string == '上报数据' then
			Http.GetUUID('api.alcedo.top:3010',
			function (x, t)
				--[[
				message = {
		...
		elseif tChat.string == '取消上报' then
			...
			Http.HttpPost('api.alcedo.top:3010',
			function (x, t)
				C.DataUpload = false
			end,'',true);
	...
	if message ~= nil and message.mes ~= '' then
	...
	then
		...
		if xpcall(function(loadDir) require( loadDir ) end, function(err) print('未查到用户数据') end, 'game/天狼星用户配置')
		then
			local userInfo = require( 'game/天狼星用户配置' )
			Http.USERNAME = userInfo['Name']
			Http.USERPASSWORD = userInfo['Password']
		end
		Http.HttpPost({operation = 'GetPoints'}, 'api.alcedo.top:3010',
			function (res, par)
				message = {
					mes = res,
					all = false
		...
		then
			...
			-- fMessage = J.Chat.GetLocalWord(sKeyType)..sVersionDate
		...
		then
			local sKeyType = J.Role.GetKeyType()
			local sUserName = J.Role.GetUserName()
			if J.Chat.GetLocalWord(sKeyType) ~= nil then
		...
		then
			if bAllNotice
			then
				bot:ActionImmediate_Chat("AI开车群:903256599, 加群和其他玩家一起挑战多倍难度.",true);
			else
...
function X.SetPushBonus( bot )
	...
	then
		bot:ActionImmediate_Chat( "由于客户端上次的更新, 经验补丁已经失效, 请重新安装补丁后重启客户端.", true)
		return
	end
	
	local bonusNoticeTable = {	

		["7.29Y3"] = "大神, 当前挑战的是三倍金钱经验夜魇AI(7.29).",
		["7.29Y2"] = "勇士, 当前挑战的是双倍金钱经验夜魇AI(7.29).",
		["7.29T2"] = "勇士, 当前挑战的是双倍金钱经验天辉AI(7.29).",
		["7.29Y1.5"] = "少年, 当前挑战的是1.5倍金钱经验夜魇AI(7.29).",	
		["7.29Y2E4M"] = "大佬, 当前挑战的是双倍经验四倍金钱夜魇AI(7.29).",	
		
		["7.30Y3"] = "大神, 当前挑战的是三倍金钱经验夜魇AI(7.30).",
		["7.30Y2"] = "勇士, 当前挑战的是双倍金钱经验夜魇AI(7.30).",
		["7.30T2"] = "勇士, 当前挑战的是双倍金钱经验天辉AI(7.30).",
		["7.30Y1.5"] = "少年, 当前挑战的是1.5倍金钱经验夜魇AI(7.30).",	

		["7.31Y3"] = "大神, 当前挑战的是三倍金钱经验夜魇AI(7.30).",
	...
	then
		...
		bot:ActionImmediate_Chat( "你好, 当前挑战的是私人定制倍数AI.", true )
		return
	end
